...
func nearestValidPoint(x int, y int, points [][]int) int {
	...
	// Time: O(n)
	// Space: O(n)
	var distanceDigit func(x int, y int, a int, b int) int
	distanceDigit = func(x int, y int, a int, b int) int {
		return abs(x - a) + abs(y - b)
	}

	ans, dis := -1, math.MaxInt64
	for i := 0 ; i < len(points) ; i++ {
		if x == points[i][0] || y == points[i][1] {
			tem := distanceDigit(x, y, points[i][0], points[i][1])
			if tem < dis {
				dis = tem
				ans = i
			}
